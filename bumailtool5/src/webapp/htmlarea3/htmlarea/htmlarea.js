...
//
// $Id: htmlarea.js,v 1.1 2005/04/27 14:30:08 janderse.umich.edu Exp $
...
HTMLArea.prototype._createLink2 = function(link, ipnum, siteid) {
	var editor = this;
	var outparam = null;
	if (typeof link == "undefined") {
		link = this.getParentElement();
...
//	this._popupDialog("link2.html"+"?siteid="+siteid+"?ipnum="+ipnum, function(param) {
	this._popupDialog("link2.html"+"?siteid="+siteid, function(param) {
		if (!param)
			return false;
		var a = link;
		if (!a) try {
...
HTMLArea.prototype._insertImage = function(image, ipnum, siteid) {
...
//	this._popupDialog("insert_image.html"+"?siteid="+siteid+"?ipnum="+ipnum, function(param) {
	this._popupDialog("insert_image.html"+"?siteid="+siteid, function(param) {
...
/*************
HTMLArea.prototype._insertAttachmentImageWithURL = function(image) {
	var editor = this;	// for nested functions
	var outparam = null;
	if (typeof image == "undefined") {
		image = this.getParentElement();
	...
	this._popupDialog("insert_Attachment_image.html"+"?siteid=5f26094d-a545-4fb0-b4f8-6ed9326b43fe", function(param) {
		if (!param) {	// user must have pressed Cancel
			return false;
		}
		var img = image;
		...
		for (field in param) {
			var value = param[field];
			switch (field) {
			    case "f_alt"    : img.alt	 = "Attachment"; break;
			}
		}
	}, outparam);
};
...
HTMLArea.prototype._insertLink = function(url)
{
  var editor = this;
  var fields = ["f_url", "f_alt", "f_align", "f_border", "f_horiz", "f_vert"];
  var values = [url, "ALT", "baseline", 0, 0, 0];
...
//		this._createLink();
		   ...
		   this._createLink2("", parts[2], parts[5]);
	    ...
	    case "insertimage": 
	    			...
	    			this._insertImage("", parts[2], parts[5]);
	    			break;
