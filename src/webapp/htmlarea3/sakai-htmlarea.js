...
var inputRichTextDir = "/mailtool/htmlarea3";
...
function sakaiRegisterButtons(config)
{
  ...
  config.registerButton("smile", //button ID,
   ...
   function(editor, id)
   {
    editor._insertSmile( inputRichTextDir + "/images/regular_smile.gif"); ///// modified
   }
  );
  ...
  config.registerButton("sad", //button ID,
   ...
   function(editor, id)
   {
     editor._insertSmile( inputRichTextDir + "/images/ed_link.gif"); ///// modified
   }
  );
  ...
  config.registerButton(
   ...
   /*
     currentEditorForPopup = editor;
     // gets called when the button is clicked
     var results = window.open( root + "/sakai_popups/insertLink.html",
      "_sakai_wysiwyg_popup", "toolbar=no,menubar=no,personalbar=no,width=650,height=380," +
      "scrollbars=no,resizable=yes");
      */
 /*     
       var results = window.open( inputRichTextDir + "/sakai-popups/insertLink.html",
       "_sakai_wysiwyg_popup", 
       "toolbar=no,menubar=no,personalbar=no,width=650,height=380,scrollbars=no,resizable=yes");
       
       var results = window.open( inputRichTextDir + "/htmlarea/popups/framelink.html",
       "_sakai_wysiwyg_popup", 
       "toolbar=no,menubar=no,personalbar=no,width=650,height=380,scrollbars=no,resizable=yes");
*/
/*******************
       ...
       var results = window.open( inputRichTextDir + "/sakai-popups/insertLink2.html"+"?siteid="+parts[5],
       "_sakai_wysiwyg_popup", 
       "toolbar=no,menubar=no,personalbar=no,width=650,height=480,scrollbars=no,resizable=yes");
********************/
alert("Coming soon - this button will be used for file uploading & picking.");
...
var host=window.parent.document.URL;
	   var parts = host.split('/');
       var results = window.open( "/access/content/group/"+parts[5]+"/?basedir=none",
       "_sakai_wysiwyg_popup", 
       "toolbar=no,menubar=no,personalbar=no,width=650,height=380,scrollbars=no,resizable=yes");
*/
/*
...
*/
/*      
       var results = window.open( "/access/content/group/816f754a-1ae4-40a7-bf3f-e3211523f74a/",
       "_sakai_wysiwyg_popup", 
       "toolbar=no,menubar=no,personalbar=no,width=650,height=380,scrollbars=no,resizable=yes");
 */
   }
  ...
  config.registerButton("sakai_createlink2", //button ID,
   "Create link", //tooltip,
   inputRichTextDir + "/images/ed_link.gif", // image
   false, // textMode,
   function(editor, id)
   {
	   var host=window.parent.document.URL;
	   var parts = host.split('/'); // get siteid

		editor._createLink2("", parts[5]);
   }
  );

  config.registerButton("sakai_insertAttachment", //button ID,
   "Insert Attachment", //tooltip,
   inputRichTextDir + "/images/ed_link.gif", // image
   false, // textMode,
   function(editor, id)
   {
		editor._insertAttachmentImageWithURL();
   }
  );
...
HTMLArea.prototype._insertSmile = function(url)
{
  var editor = this;
  var fields = ["f_url", "f_alt", "f_align", "f_border", "f_horiz", "f_vert"];
  var values = [url, "ALT", "baseline", 0, 0, 0];
